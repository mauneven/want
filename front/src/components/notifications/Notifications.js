...
export default function Notifications() {
  ...
  const fetchPostName = async (postId) => {
    ...
    const response = await fetch(`http://ec2-34-192-108-182.compute-1.amazonaws.com:4000/api/posts/${postId}`);
  ...
  useEffect(() => {
    const fetchNotifications = async () => {
      const response = await fetch('http://ec2-34-192-108-182.compute-1.amazonaws.com:4000/api/notifications', {
        credentials: 'include',
      });
  ...
  const handleNotificationClick = async (notificationId) => {
    ...
    await fetch(`http://ec2-34-192-108-182.compute-1.amazonaws.com:4000/api/notifications/${notificationId}/read`, {
      method: 'PATCH',
      credentials: 'include',
    });
