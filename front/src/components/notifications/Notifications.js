...
export default function Notifications() {
  ...
  const fetchPostName = async (postId) => {
    ...
    const response = await fetch(`https://ec2-100-25-111-207.compute-1.amazonaws.com/api/posts/${postId}`);
  ...
  useEffect(() => {
    const fetchNotifications = async () => {
      const response = await fetch('https://ec2-100-25-111-207.compute-1.amazonaws.com/api/notifications', {
        credentials: 'include',
      });
  ...
  const handleNotificationClick = async (notificationId) => {
    ...
    await fetch(`https://ec2-100-25-111-207.compute-1.amazonaws.com/api/notifications/${notificationId}/read`, {
      method: 'PATCH',
      credentials: 'include',
    });
