...
const PostsList = ({ locationFilter, userIdFilter, searchTerm, categoryFilter }) => {
  ...
  const fetchPostsByLocation = async () => {
    const response = await fetch("https://ec2-100-25-111-207.compute-1.amazonaws.com:4000/api/posts");
  ...
  const handleReportPost = async (postId, description) => {
    try {
      const response = await fetch(`https://ec2-100-25-111-207.compute-1.amazonaws.com:4000/api/report/post/${postId}`, {
        method: 'POST',
        headers: {
