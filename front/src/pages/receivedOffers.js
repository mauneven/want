...
export default function ReceivedOffers() {
  ...
  useEffect(() => {
    const fetchReceivedOffers = async () => {
      const response = await fetch('http://ec2-3-89-21-249.compute-1.amazonaws.com:4000/api/received', {
        credentials: 'include',
      });
  ...
  const handleReportOffer = async (offerId, description) => {
    try {
      const response = await fetch(`http://ec2-3-89-21-249.compute-1.amazonaws.com:4000/api/offers/${offerId}/report`, {
        method: 'POST',
        headers: {
  ...
  const handleDeleteOffer = async () => {
    try {
      const response = await fetch(`http://ec2-3-89-21-249.compute-1.amazonaws.com:4000/api/${selectedOfferId}`, {
        method: 'DELETE',
        credentials: 'include',
      });
