...
const EditProfile = () => {
  ...
  useEffect(() => {
    fetch('http://ec2-3-89-21-249.compute-1.amazonaws.com:4000/api/user', { credentials: 'include' })
      .then((response) => {
        if (!response.ok) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await fetch('http://ec2-3-89-21-249.compute-1.amazonaws.com:4000/api/users/me', {
        method: 'PUT',
        credentials: 'include',
        body: formData
      });
