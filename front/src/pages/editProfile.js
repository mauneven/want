...
const EditProfile = () => {
  ...
  useEffect(() => {
    fetch('https://ec2-100-25-111-207.compute-1.amazonaws.com/api/user', { credentials: 'include' })
      .then((response) => {
        if (!response.ok) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await fetch('https://ec2-100-25-111-207.compute-1.amazonaws.com/api/users/me', {
        method: 'PUT',
        credentials: 'include',
        body: formData
      });
