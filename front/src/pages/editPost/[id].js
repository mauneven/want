...
const EditPost = () => {
    ...
    useEffect(() => {
        const checkLoggedIn = async () => {
            const response = await fetch('http://ec2-34-192-108-182.compute-1.amazonaws.com:4000/api/is-logged-in', {
                credentials: 'include',
            });
    ...
    useEffect(() => {
        if (id) {
            fetch(`http://ec2-34-192-108-182.compute-1.amazonaws.com:4000/api/posts/${id}`)
                .then((response) => response.json())
                .then((data) => {
                    // Verificar si el usuario actual es el propietario del post
                    fetch('http://ec2-34-192-108-182.compute-1.amazonaws.com:4000/api/user')
                        .then((res) => res.json())
                        .then((user) => {
