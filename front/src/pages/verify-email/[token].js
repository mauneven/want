...
const VerifyEmail = () => {
  ...
  useEffect(() => {
    if (token) {
      fetch(`http://ec2-100-25-111-207.compute-1.amazonaws.com:4000/api/verify/${token}`, { method: 'POST' })
        .then(async (response) => {
  ...
  const handleResendVerification = async (e) => {
    ...
    const response = await fetch(`http://ec2-100-25-111-207.compute-1.amazonaws.com:4000/api/resend-verification`, {
      method: 'POST',
      headers: {
