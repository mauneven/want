...
export default function MyPosts() {
  ...
  useEffect(() => {
    const fetchMyPosts = async () => {
      const response = await fetch('http://ec2-3-89-21-249.compute-1.amazonaws.com:4000/api/my-posts', {
        credentials: 'include',
      });
  ...
  const handleDeletePost = async () => {
    try {
      const response = await fetch(`http://ec2-3-89-21-249.compute-1.amazonaws.com:4000/api/posts/${selectedPostId}`, {
        method: 'DELETE',
        credentials: 'include',
      });
