  ...
  useEffect(() => {
    const checkLoggedInAndBlockedAndVerified = async () => {
      const loggedInResponse = await fetch(
        `${process.env.NEXT_PUBLIC_API_BASE_URL}/api/is-logged-in`,
        {
          credentials: "include",
        }
      );
      ...
      const blockedResponse = await fetch(
        `${process.env.NEXT_PUBLIC_API_BASE_URL}/api/is-blocked`,
        {
          credentials: "include",
        }
      );
  ...
  const handleSubmit = async (event) => {
    ...
    if (response.ok) {
      ...
      if (response.status === 409) {
        ...
        const responseData = await response.json();
        setAlertMessage(
          responseData.error || t("login.invalidCredentialsErrorMessage")
        );
      } else {
      ...
      <div className="card login-form want-rounded pt-3 pb-3">
        <div className="card-body">
          <div className="container">
            ...
            {!isLogin && (
              <p className="text-small small text-price m-0">
                {t("login.termsandprivacycheck")}
                <span
                  className="links"
            ...
            <div>
              ...
              <button onClick={toggleForm} className="want-button m-2">
                {isLogin ? t("login.signupLink") : t("login.loginLink")}
              </button>
            </div>
            <Link href="/recovery">
              <span className="divhover want-color">
